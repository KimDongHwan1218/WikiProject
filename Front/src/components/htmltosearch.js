export default function htmltosearch(input){
  ...
  function check_paragraph(input){
    ...
    if(input.match(heading)) return(input.match(heading)+'')
    else if(input.match(paragraph)) return(input.match(paragraph)+'')
    else if(input.match(smallparagraph)) return(input.match(smallparagraph)+'')
    else if(input === '') return('') // style로 간격 줄여주기
    return(input)
  }
  ...
  function check_dividingline(input){
    var dividing = /^----$/
    console.log("check_dividingline:", input)
    if(input.match(dividing)) return ('')
    return(input)
  }
  ...
  function textshape(input){
    ...
    while(output.match(bold_italic)){
      output = output.replace(bold_italic, '')
      output = output.replace(bold_italic, '')
    }
